...
class ResponseOutput(BaseModel):
    ...
    links: Dict[str, str] = Field(
        description='{"link1":<dalle3 prompt to generate the image>,...}'
    )
...
def generate_article(topic_name, topic_description, llm):
    ...
    question_context_chain = (
        generate_article_prompt_template | llm | response_output_parser
    )
    ...
    if links:
        for link, description in links.items():
            imageurl, _ = generate_image_from_text(
                llm=llm, text=description, text_type="description", image_type="image"
            )
            local_imageurl = download_and_save_image(
                url=imageurl, topic_name=topic_name
            )
